...
When /^the hidden field "([^\"]*)" is set to "([^\"]*)"$/ do |field, value|
...
Then /^(?:|I )should be on (.+) with query string (.+)$/ do |page_name, query_string|
  current_path = URI.parse(current_url).select(:path, :query).compact.join('?')
  desired_path = [page_name, query_string.sub(/^\?/,'')].join('?')
  if defined?(Spec::Matchers)
    current_path.should == path_to(desired_path)
  else
